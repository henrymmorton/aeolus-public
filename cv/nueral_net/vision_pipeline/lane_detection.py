...
class Lane:
    ...
    def __init__(self, timestamp):
        ...
        # The birds eye image that tthe lane is derived from
        self.birds_eye_image = None
...
def lane_detection_pipeline(persepctive_transformer, inference, image_time, image = None, plot = False):
    ...
    if missing_left_or_right:
        ...
        logger.info(f"LANE DETECTION: Successfully detected left and right lane {round(time.time() - t_mark, 5)}s")
        t_mark = time.time()
    ...
    current_lane.birds_eye_image = birds_eye
