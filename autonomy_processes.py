...
def copy_to_local_queue(local_queue, ref_queue):
    """
    Copies objects from one multiprocessing queue to a local queue
    """
    temp_list = []
    # Drain all the items into a temporary list
    while True:
        try:
            i_queue_obj = ref_queue.get_nowait()
            temp_list.append(i_queue_obj)
        except queue.Empty:
            break

    # Refill the multiprocessing queue and the local queue from the temporary list
    for i_queue_obj in temp_list:
        ref_queue.put(i_queue_obj)
        local_queue.put(i_queue_obj)

def dump_to_local_queue(local_queue, ref_queue):
    """
    Copies objects from one queue to another and empties the original
    """
    while True:
        try:
            i_queue_obj = ref_queue.get_nowait()
...
def state_estimation_control_process(start_flag, timeout_flag, lane_queue, teensy_state_queue, vehicle_state_queue, mp_speed, mp_steering_angle, speed_matrix, log_queue):
    ...
    while not timeout_flag.is_set():
        ...
        if (not lane_queue.empty()) and (not teensy_state_queue.empty()):
            # Make a new queue and dump all teensy state information to it. Then empty the current queue
            recent_teensy_state_queue = queue.Queue()
            dump_to_local_queue(recent_teensy_state_queue, teensy_state_queue)

            # Get the most recent lane detection information
            while not lane_queue.empty():
                new_lane_obj = lane_queue.get()

            vehicle_state.updateState(new_lane_obj, recent_teensy_state_queue, mp_steering_angle, plot=False)
        ...
        # Case where new teensy measurements are available but no new lane object are available
            ...
            # Make a new queue and copy all teensy state information to it.
            recent_teensy_state_queue = queue.Queue()
            copy_to_local_queue(recent_teensy_state_queue, teensy_state_queue)

            vehicle_state.updateState(None, recent_teensy_state_queue, mp_steering_angle, plot=False)
        ...
        logger.debug(f"STATE ESTIMATION/CONTROL PROCESS STATUS: Computed control outputs [speed: {control_speed}m/s, steering: {new_steering_angle}]")
