...
model_path = "/home/henry/git/aeolus_dev/test/test_models/erfnet_small.ts"
...
class Autonomy:
    ...
    def teensy_communication_process(self, timeout_flag):
        ...
        param timeout_flag: A threading event signal when to halt the thread
        """
        tsr_loop_count = 0
    ...
    def teensy_state_spoofing_process(self, start_flag, timeout_flag, teensy_state_store):
        ...
        while not timeout_flag.is_set():
            current_teensy_state = teensy_faker.get_state_from_state_store(teensy_state_store)
            if current_teensy_state:
                self.teensy_state_queue.put(current_teensy_state)
...
def prime_autonomy_pipeline(aeolus, image_stream = None):
    ...
    initial_state.initializeState(lane_object, plot=True)
...
def spin_up_spoofed_autonomy(speed_matrix, recording_path):
    """
    ...
    image_stream = camera_faker.StreamFaker(image_path)

    # Spin up the spoofed teensy state store
    state_store = teensy_faker.spin_up_teensy_state_store(teensy_state_path)
